...
// ✅ Raster storage directory (set via environment variable)
...
// ✅ Route: GET /update-password – Render Update Password page (protected)
router.get("/update-password", adminAuthMiddleware, (req, res) => {
  try {
    res.status(200).render("adminUpdatePassword", {
      full_name: req.user.full_name,
    });
  } catch (error) {
    res.status(500).json({ message: error.message, title: "Error", icon: "error" });
  }
});

// ✅ Route: POST /update-password – Handle password change (protected)
// upload.none() parses multipart/form-data text fields (same as all other admin form routes)
router.post("/update-password", adminAuthMiddleware, upload.none(), async (req, res) => {
  try {
    ...
    const updateClient = await poolUser.connect();
    await updateClient.query(
      "UPDATE admins SET password = $1 WHERE email = $2",
      [hashedNew, req.user.email]
    );
...
// ──────────────────────────────────────────────────────────────
// ✅ Route: GET /admin/raster/untracked
//    Lists all .tif files in untraked_RASTER_DIR (staging / incoming
//    directory for pscp uploads). No catalog cross-check needed —
//    everything in that dir is by definition unregistered.
// ──────────────────────────────────────────────────────────────
const UNTRACKED_DIR = process.env.untraked_RASTER_DIR;

router.get("/raster/untracked", adminAuthMiddleware, async (req, res) => {
  try {
    if (!UNTRACKED_DIR || !fs.existsSync(UNTRACKED_DIR)) {
      return res.json({ files: [] });
    }

    const files = fs.readdirSync(UNTRACKED_DIR)
      .filter(f => f.toLowerCase().endsWith(".tif"))
      .map(f => path.basename(f, path.extname(f)))   // strip .tif
      .sort();

    return res.json({ files });
  } catch (err) {
    console.error("[raster/untracked]", err.message);
    return res.status(500).json({ error: "Failed to list files" });
  }
});

// ──────────────────────────────────────────────────────────────
// ✅ Route: POST /admin/raster/manual-entry
//    Moves a .tif from untraked_RASTER_DIR → RASTER_DIR under a
//    user-supplied catalog name, then inserts a catalog row.
//
//    Body: { source_file, file_name }
//      source_file  – original filename (no ext) in untraked_RASTER_DIR
//      file_name    – desired catalog name (no ext) in RASTER_DIR
//    type='raster', theme='raster', srid='4326' are fixed.
// ──────────────────────────────────────────────────────────────
const SAFE_NAME = /^[a-zA-Z0-9_]+$/;

router.post("/raster/manual-entry", adminAuthMiddleware, async (req, res) => {
  try {
    const { source_file, file_name } = req.body;

    // ── Presence check ────────────────────────────────────────
    if (!source_file || !file_name) {
      return res.status(400).json({
        title: "Missing fields",
        message: "Both source_file and file_name are required.",
        icon: "warning",
      });
    }

    // ── Input validation ──────────────────────────────────────
    if (!SAFE_NAME.test(source_file) || !SAFE_NAME.test(file_name)) {
      return res.status(400).json({
        title: "Invalid input",
        message: "Names may only contain letters, numbers, and underscores.",
        icon: "warning",
      });
    }

    // ── Source file must exist in staging dir ─────────────────
    if (!UNTRACKED_DIR || !fs.existsSync(UNTRACKED_DIR)) {
      return res.status(500).json({
        title: "Configuration error",
        message: "untraked_RASTER_DIR is not configured or does not exist.",
        icon: "error",
      });
    }
    ...
    if (!fs.existsSync(srcPath)) {
      return res.status(404).json({
        ...
        message: `${source_file}.tif was not found in the incoming directory.`,
        icon: "error",
      });
    }
    ...
    if (fs.existsSync(destPath)) {
      return res.status(409).json({
        title: "Name conflict",
        message: `${file_name}.tif already exists in the raster store. Choose a different name.`,
        icon: "warning",
      });
    }
    ...
    try {
      ...
      if (rowCount > 0) {
        return res.status(409).json({
          ...
          message: `${file_name} already has a catalog entry.`,
          icon: "warning",
        });
      }
      ...
      await client.query(
        `INSERT INTO catalog
         ...
         VALUES ($1, 'raster', 'raster', '4326', false, false)`,
        [file_name]
      );
    } finally {
      client.release();
    ...
    console.log(`[manual-entry] ${source_file}.tif → ${file_name}.tif registered.`);

    return res.json({
      title: "Registered",
      message: `${source_file}.tif has been moved and registered as "${file_name}" in the catalog.`,
      icon: "success",
    });
  } catch (err) {
    console.error("[raster/manual-entry]", err.message);
    return res.status(500).json({
      title: "Server error",
      message: "Something went wrong. Please try again.",
      icon: "error",
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const RASTER_DIR = process.env.RASTER_DIR;
      const rasterPath = path.join(RASTER_DIR, `${file_name}.tif`);
